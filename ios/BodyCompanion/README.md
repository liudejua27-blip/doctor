...
`7d8bf59` 的 Swift Core 94 tests、0 failures，以及本地 iPhone 17 Pro Max / iOS 26.5 的 13 项 HOST-T-001～015 UI 流程均是历史 Simulator 收据；HOST-T-015 仅在 `UICTContentSizeCategoryAccessibilityXXXL` 下验证文字部位 Sheet、今天两张展示性情境卡和结构化页两组成对动作的局部结构。历史 SHA `98a4200` / run `31314859140` 的 H015 是文字部位 Sheet 关闭后 `body-map.next` 存在但不可点击。SHA `544c39a` 前的 iPhone 17 Pro / iOS 26.5 定向 HOST-T-015 1 passed、0 failed、0 skipped（86.0s）及完整 `run_internal_ios_host_tests.sh` exit 0（402.313s；脚本安静输出，不记录精确 XCTest 数）也仅为历史收据。历史 run `31318855810`（SHA `544c39a`）中 Backend and contracts 成功，iOS 仅 internal Host smoke 失败：12 passed、1 failed、0 skipped，HOST-T-015 报 `Expected element to exist: body-map.next Button`（exit 65）。最新远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；唯一 H015 未在 5 秒内满足 `body-map.marker-count-summary` 的旧计数文字断言，不能归因位置丢失。当前工作树使 active/empty count ID 在真实 count pill 上互斥呈现，Host 仅检查摘要 identifier 存在；CTA 按 stable identifier 的任意元素查询仍验证 `body-map.next` 存在、`isHittable` 和 `tap`。本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2 passed、0 failed、0 skipped（138.685s），完整 `run_internal_ios_host_tests.sh` exit 0（441.966s；脚本安静输出，不记录精确 XCTest 数）；本次修复尚未推送、重试 Pending。`0105cd7` 的 12 项流程、包括“更多感觉（22 项）”选中“麻木”和两位置 unknown 边界，以及 run `31307042209` 在 iPhone 16 / iOS Simulator 18.5 的远端重建保留为历史证据。它们只形成未确认、显式位置关联的草稿，不读取网络、资料、历史或 Agent，不写正式档案。`9fc54d4` 的 87 tests、10 项 Area-only UI flow 与 run `31303340128` 的成功记录也保留为历史证据；此前完整 Host suite 还在 run `31301067572` 中成功重建。任何远端 smoke 或 probe 都只验证受控 Simulator 路径，不把结果外推为真实设备、VoiceOver、完整 Dynamic Type、深色/高对比度、Reduce Motion、3D 性能/碰撞、签名或生产资产通过。详细边界见：

远端兼容性收据：SHA `9b6e10a` / run `31348638741` 在 Xcode 16.4 的 UI test 编译阶段因 `XCTIssue.isFailure` API 不可用而失败，0 项 UI test 执行；artifact 已上传但没有测试附件。当前修复改为跨版本 `XCTIssue.type == .assertionFailure`，待提交、推送与重试。
