...
**面向运动与久坐办公人群的身体不适表达、恢复决策与沟通助手：把一次主观感觉转化为机器可理解、用户可确认、可行动与可比较的数据。**

### 1.4 首发主闭环

首发体验围绕用户发生一次不适后的真实任务，而不是把身体地图、档案或开放聊天做成彼此孤立的功能：

记录这次不适 → 点击 2D/3D 人体上的感觉位置 → 说明本次更接近训练、久坐/工作、日常活动还是说不清 → 完成确定性安全分流 → AI 身体助手一次一问地补足高价值事实 → 用户复核“你告诉我的 / AI 整理，待你确认 / 本次经授权参考” → 根据安全等级显示审核过的下一步 → 批准保存、复查、比较或生成沟通摘要。

其中“下一步”可以涵盖经审核的观察、训练/工作负荷调整、低风险舒适活动、一般恢复支持和升级条件；它不是疾病结论、治疗处方、复出许可，亦不能由模型临时编写。详细范围见 FEAT-COMP-01 与 ADR-0019。
...
3. **AI 身体助手**：当前采集会话、情境化智能追问、事实确认、安全行动与下一步。
...
- 将本次训练负荷、久坐/工作负荷、日常活动或“说不清”作为可撤回的会话情境，只用于问题排序和审核内容筛选，不作为病因、风险等级或长期档案事实；
- 从位置、感觉、强度、时间、诱发/缓解、功能、背景和安全信号中选择下一条最有价值的问题；
- 避免重复询问已有且仍有效的确认事实；
- 把自由文本提取为强类型 `UnconfirmedInput`，让用户确认而非自动入档；
- 清楚区分用户事实、设备/文件信息、AI 推断和可能相关因素；
- 在可选资料读取前后显示本次资料类别、来源、时间范围、用途和实际使用结果；拒绝可选资料后仍能完成核心记录；
- 只能引用审核内容库中的行动建议；
- 在模型失败时转入确定性的表单/模板路径。

Agent 禁止：诊断、疾病概率、排除严重疾病、开药、调整用药、即时发明治疗方案、自由编造动作/拉伸、具体食物/补剂/剂量建议、越权读取资料、未经确认写入长期档案。
...
用户侧默认名称为“身体信号报告”。在用户主动选择接收者与字段范围后，同一不可变报告可采用“就诊沟通摘要”或“训练沟通摘要”的显示类型；它们不是病例、病历、诊断报告、医疗证明或 EMR 写入。
...
首发必须将运动与久坐办公两类用户的核心任务组织为同一评估会话：

1. 用户从“记录这次不适”开始，在 2D/3D/列表中表达主观位置；
2. 用户选择或确认本次情境：训练/运动负荷、久坐/工作负荷、日常活动或说不清；
3. 用户可选择“仅使用本次记录”，或逐来源授权当前记录、活动/工作背景与同部位历史的最小摘要；
4. 确定性安全规则先于普通 Agent 运行；普通聊天不能绕过 R0/R1/R2、未解决安全或未覆盖场景；
5. AI 身体助手一次只问一个高价值问题，并持续可见地区分“你告诉我的”“AI 整理，待你确认”“本次经授权参考”“仍不确定”；
6. 只有安全、已确认事实、适用场景和审核内容全部满足时，才显示绑定内容 ID、事实依据、停止/升级条件的行动计划；
7. 用户经两次语义不同的确认后保存身体信号记录，并可创建复查、比较趋势、生成身体信号报告、就诊沟通摘要或训练沟通摘要。

会话情境、资料使用收据、行动内容引用和报告显示类型属于跨端契约变更，须在 ADR-0019 获批后再更新 API/Schema 并实现。详细范围和验收见 FEAT-COMP-01。

## 7. 体验分层

### 首次 30 秒快捷路径

位置 → 本次情境 → 感觉/强度 → 关键诱发/功能影响 → 安全必要问题 → AI 一次一问补足缺失事实 → 确认并生成摘要。
...
- 默认读取用户全部健康资料；
- 用用户健康数据训练模型的捆绑同意。
- 把输出称为病例、病历、医疗证明或已获专业人员审核的结论；
- 在没有独立临床/营养/地区审核时，针对不适推荐具体食物、补剂、剂量或自创动作。
...
| RISK-007 | 用户把“缓解动作/吃什么”理解为处方或疗效承诺 | 行动只来自审核内容库；R0–R2 抑制普通自我管理；首发关闭具体食物、补剂和剂量 | 临床安全 + 营养审核 + 产品 | GATE-03 |
...
- 运动/久坐两类用户均能理解“本次情境”“资料使用范围”“AI 候选”和“审核行动”之间的区别，且拒绝可选资料仍可完成记录。
- 具体动作、一般恢复支持或营养相关内容在未通过临床/营养、地区和内容版本门禁时保持关闭。

## 14. 变更记录

| 日期 | 变更 | 说明 |
|---|---|---|
| 2026-08-09 | 1.1.0-draft | 将首发体验明确收拢为运动/工作不适的对话式决策闭环；新增 PRD-F11，并固定报告命名、资料可见性与审核行动边界。 |
