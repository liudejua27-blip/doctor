...
| iOS Swift | `0105cd7`：94 tests、0 failures；iOS `BodyCompanionIOS` SDK build、`check_internal_ios_host.py` 与本地 iPhone 17 Pro / iOS 26.5 的 12 项 `BodyCompanionInternal` UI flow 通过，覆盖 HOST-T-001～014，包括“更多感觉”选中“麻木”和两类 unknown 文案。run `31307042209` 已在 iPhone 16 / iOS Simulator 18.5 成功重建 Swift 94、SDK build、Host boundary 与 internal Host smoke。`9fc54d4` 的 87 tests、10 项 Area-only UI flow、历史 7 项本地/远端 Host smoke、`79f1f36` 本地 9 项候选 probe 与 `ea85c68` / run `31301067572` 的远端完整 Host suite 保留为历史证据 | 证明内部 Simulator 可安装/启动的 P0 壳、基础回退、文字目录契约、感觉目录入口和语义感觉修订的本地安全边界；不能替代真机 UI、真实无障碍或临床验收 |
| 后端 Python | 194 tests、0 failures；P2A 聚焦 19 tests | 受控时钟回归已修复；只证明合成/内存样机 |
| Git/CI | `codex/initial-git-ci-baseline` 受保护默认分支合并提交 `2bb8e8a`；本轮复刻切片提交 `6f232b4`、内部 Host `5dddd83`、地图投影边界修复 `a796f0b`、候选 3D 运行时防护 `79f1f36`、证据提交 `ea85c68`、文字部位 Area-only 修复 `9fc54d4`、完整感觉与安全修订边界 `0105cd7`；双作业 workflow；Python 3.11 constraints | run `31295533318`、run `31296924827`、`ea85c68` 的 run `31301067572`、`9fc54d4` 的 run `31303340128` 与 `0105cd7` 的 run `31307042209` 均已在远端完成 Backend and contracts 与相应 iOS Host smoke；后者重建 Swift 94、SDK build、Host boundary 和 internal Host smoke。任何结果均不等于真机或发布验收 |
