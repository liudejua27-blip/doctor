...
| iOS Swift | `7d8bf59`：94 tests、0 failures；iOS `BodyCompanionIOS` SDK build、`check_internal_ios_host.py` 与本地 iPhone 17 Pro Max / iOS 26.5 的 13 项 `BodyCompanionInternal` UI flow 通过，覆盖 HOST-T-001～015。HOST-T-015 只以 `UICTContentSizeCategoryAccessibilityXXXL` 验证局部纵向/可达结构。历史 SHA `98a4200` / run `31314859140` 仍只失败于 H015（CTA 存在但不可点击）。SHA `544c39a` 前的本地 H015 1/0/0（86.0s）和完整 Host script exit 0（402.313s）仅为历史收据。最新 SHA `544c39a` / run `31318855810`：Backend and contracts 成功，iOS 仅 Host smoke 失败，12 passed、1 failed、0 skipped；H015 报 `Expected element to exist: body-map.next Button`（exit 65）。CI 无 artifact/accessibility hierarchy，根因未知。当前实施已完成底部独立非滚动安全内容区、marker-count summary 的一项位置先验、stable-id any-element CTA exists/hittable/tap，非 `safeAreaInset` 或 `Button` class 契约；本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送、远端重试 Pending。`0105cd7` 的 12 项完整感觉 Host 流程和 run `31307042209` 的远端 iPhone 16 / iOS Simulator 18.5 重建保留为历史证据；`9fc54d4` 的 87 tests、10 项 Area-only UI flow、历史 7 项本地/远端 Host smoke、`79f1f36` 本地 9 项候选 probe 与 `ea85c68` / run `31301067572` 的远端完整 Host suite 亦保留为历史证据 | 证明内部 Simulator 可安装/启动的 P0 壳、基础回退、文字目录契约、感觉目录入口、语义感觉修订的本地安全边界及局部 accessibility-size 结构；不能替代真机 UI、完整 Dynamic Type、VoiceOver、深色/高对比度、Reduce Motion 或临床验收 |
| 后端 Python | 194 tests、0 failures；P2A 聚焦 19 tests | 受控时钟回归已修复；只证明合成/内存样机 |
| Git/CI | `codex/initial-git-ci-baseline` 受保护默认分支合并提交 `2bb8e8a`；本轮复刻切片提交 `6f232b4`、内部 Host `5dddd83`、地图投影边界修复 `a796f0b`、候选 3D 运行时防护 `79f1f36`、证据提交 `ea85c68`、文字部位 Area-only 修复 `9fc54d4`、完整感觉与安全修订边界 `0105cd7`、accessibility-size 结构优化 `7d8bf59`、三次已提交 Host smoke 修复 `7e64833` / `e0f1089` / `0ed0563`；双作业 workflow；Python 3.11 constraints | run `31295533318`、run `31296924827`、`ea85c68` 的 run `31301067572`、`9fc54d4` 的 run `31303340128` 与 `0105cd7` 的 run `31307042209` 均已在远端完成 Backend and contracts 与相应 iOS Host smoke。历史 run `31314859140`（SHA `98a4200`）的 H015 是 CTA 存在但不可点击。最新 run `31318855810`（SHA `544c39a`）：Backend and contracts 成功，iOS 仅 Host smoke 失败，12 passed、1 failed、0 skipped，`Expected element to exist: body-map.next Button`（exit 65）；CI 无 artifact/accessibility hierarchy，不能归因。SHA `544c39a` 前的 1/0/0（86.0s）和完整 Host script exit 0（402.313s）仅历史。当前修复已完成：独立非滚动安全内容区、marker-count 一项位置先验、stable-id any-element CTA exists/hittable/tap，非 `safeAreaInset` / `Button` class 条件；本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送、远端重试 Pending。任何结果均不等于真机或发布验收 |
...
按 REL-01 顺序执行：内部 Host 与历史远端 CI 基线已完成；最新远端 SHA `544c39a` / run `31318855810` 仍失败于 HOST-T-015（`Expected element to exist: body-map.next Button`，exit 65），尽管 Backend and contracts 成功。因 CI 无 artifact/accessibility hierarchy，不能诊断是选择状态还是 AX 投影/元素类型。当前工作树已实现底部独立、非滚动安全内容布局区，并更新 H015 为文字选择后先验证 `body-map.marker-count-summary` 恰有一项位置，再以 stable-id any-element 验证 CTA exists/hittable/tap；不得以 `safeAreaInset` 或 `Button` class 定义跨 OS 成功。本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送、远端重试 Pending。之后再关闭责任/地区、临床安全、Provider、身份同意数据库、真实资产与客户端、公开纵向联调、真实用户/设备验证门禁。
