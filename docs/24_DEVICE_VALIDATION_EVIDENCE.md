...
7. `7d8bf59` 在本地 iPhone 17 Pro Max / iOS 26.5 完成 13 项 UI 流程与 94 项 Swift Core：HOST-T-015 只在 `UICTContentSizeCategoryAccessibilityXXXL` 下验证文字部位 Sheet、今天页两张展示性情境卡的稳定 ID/纵向分支和结构化页成对动作的局部结构。前三次远端失败历史保持不变；SHA `98a4200` 的 run `31314859140` 是第四次 H015 失败，文字部位 Sheet 关闭后 CTA 存在但不可点击。SHA `544c39a` 前的本地定向 H015 1/0/0（86.0s）和完整 Host script exit 0（402.313s）仅为历史收据。最新 SHA `544c39a` 的 run `31318855810` 中 Backend and contracts 成功，iOS 仅 Host smoke 失败：12 passed、1 failed、0 skipped，HOST-T-015 报 `Expected element to exist: body-map.next Button`（exit 65）。CI 无 artifact/accessibility hierarchy，不能断定文字选择未保留位置，或仅为 AX 投影/元素类型。当前修复已实现：底部独立、非滚动安全内容布局区，文字选择后先验证 `body-map.marker-count-summary` 的一项位置，再按 stable-id any-element 验证 CTA 存在、`isHittable` 和 `tap`；不要求具体 `safeAreaInset` API 或 `Button` class。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送、远端重试 Pending。它不构成 VoiceOver、完整最大 Dynamic Type、深色/高对比度、Reduce Motion、真实设备、碰撞、性能或资产审核证据。

因此，Sheet、VoiceOver、Dynamic Type、Reduce Motion、3D 性能、RealityKit 碰撞命中和候选资产的生产批准均保持 **Pending/Blocked**。当前工作树的布局与测试修复已在 iPhone 17 Pro / iOS 26.5 形成 targeted H015 + MoreSensations 2/0/0（138.1s）和完整 internal Host script exit 0（426.285s）的内部 Simulator 收据，但不改变最新远端 H015 失败；远端尚未推送、重试 Pending，且不关闭上述任一设备门禁。2D、部位列表和安全入口继续是唯一可外推的可用路径；候选 USDZ 继续为 `candidate`，不得发布。
...
| GitHub Actions run `31318855810` | SHA `544c39a`：Backend and contracts 成功；iOS 仅 internal Host smoke 失败，12 passed、1 failed、0 skipped；HOST-T-015 报 `Expected element to exist: body-map.next Button`（exit 65）。CI 未上传 artifact 或 accessibility hierarchy | 最新失败记录；不能断定文字选择未保留位置还是 AX 投影/元素类型，也不证明无障碍、真机或发布通过 |
...
| Dynamic Type | **Historical Simulator structural smoke / device blocked** | 使用 `.body/.headline/.caption/.footnote` 等语义字体；`7d8bf59` 在 `UICTContentSizeCategoryAccessibilityXXXL` 下验证指定文字部位 Sheet、两张展示性情境卡和两组结构化成对动作的局部结构。历史 `31314859140` 为 CTA 存在但不可点击；SHA `544c39a` 前的本地 H015 1/0/0（86.0s）和 full Host script exit 0（402.313s）仅历史。最新 `31318855810`（SHA `544c39a`）中 H015 报 `Expected element to exist: body-map.next Button`（12/1/0，exit 65）；无 CI artifact/hierarchy，根因未知。下一次布局/测试修复、本地复测和远端重试 Pending，并规定独立非滚动安全内容区、marker-count 一项位置先验和 stable-id any-element CTA exists/hittable/tap，非 `safeAreaInset`/`Button` class 契约 | 最大可访问字号和横屏下不截断标题、位置摘要、提示、结构化程度控件和按钮；确认 Sheet 可滚动且关键操作仍可达 |
...
| DEVICE-FINDING-007 | P2 | 2026-08-09 已从 `MarkEditor` 移除程度 Slider；结构化 `SignalIntakeScreen` 的程度控件虽有可见“程度”标签与 accessibility value，仍未经过真机 VoiceOver/最大 Dynamic Type 验证。历史 `31314859140` 的 H015 为 CTA 存在但不可点击；SHA `544c39a` 前的本地 H015 1/0/0（86.0s）和 full Host script exit 0（402.313s）仅历史。最新 `31318855810`（SHA `544c39a`）中 Backend and contracts 成功，但 iOS Host smoke 的 H015 报 `Expected element to exist: body-map.next Button`（12/1/0，exit 65）；CI 无 artifact/accessibility hierarchy，不能归因。当前修复已实施并在 iPhone 17 Pro / iOS 26.5 通过 targeted H015 + MoreSensations（2/0/0，138.1s）及完整 Host script（exit 0，426.285s）：独立非滚动安全内容区、marker-count 一项位置先验与 stable-id any-element CTA exists/hittable/tap；不要求 `safeAreaInset` 或 `Button` class。远端重试仍 Pending。位置摘要、完整页面、深色/高对比度和横屏仍未经过设备验证。 | 真机 VoiceOver/Dynamic Type 验证结构化程度控件；必要时取消摘要硬截断，并保留部位列表等价路径。 |
...
本轮决定：**记录历史基础 `Simulator host smoke passed`、`79f1f36` 的本地候选 loader lifecycle probe passed、`ea85c68` / run `31301067572` 的远端完整 Host smoke passed，以及 `9fc54d4` 的本地文字部位 Area-only Simulator flow 与 run `31303340128` 的远端 Simulator 重建成功；另记录 `0105cd7` 的本地完整感觉/感觉修订安全边界 Simulator flow 及 run `31307042209` 的远端重建成功。历史 SHA `98a4200` / run `31314859140` 的 H015 为 CTA 存在但不可点击；SHA `544c39a` 前的本地定向 H015 1/0/0（86.0s）与完整 Host script exit 0（402.313s）仅为历史收据。最新 SHA `544c39a` / run `31318855810` 中 Backend and contracts 成功，iOS 仅 Host smoke 失败（12 passed、1 failed、0 skipped），H015 报 `Expected element to exist: body-map.next Button`（exit 65）；CI 无 artifact/accessibility hierarchy，不能断定选择丢失或 AX 投影/元素类型。当前修复已实施：底部独立、非滚动安全内容布局区，文字选择后先 marker-count 一项位置，再 stable-id any-element CTA exists/hittable/tap；不将 `safeAreaInset` 或 `Button` class 固定为跨 OS 契约。iPhone 17 Pro / iOS 26.5 上 targeted H015 + MoreSensations 为 2/0/0（138.1s），完整 Host script exit 0（426.285s）；远端重试仍 Pending。** 不批准生产 3D，不声称真机通过或无障碍通过，不关闭 GATE-06/GATE-07/GATE-08。设备和正式 App target 到位前，生产路径保持 2D/列表 fail-closed；候选资产和 3D loader 仅限内部 prototype flag。
