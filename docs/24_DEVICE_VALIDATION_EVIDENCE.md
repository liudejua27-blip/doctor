...
| 版本 | 1.5.12 |
| 状态 | Current local internal Simulator Host receipt passed; latest remote Host smoke failed and retry is not pushed; physical device and production 3D validation blocked |
| 执行日期 | 2026-08-08（设备尝试）；2026-08-09（Host 基线、候选 probe、文字部位切片、完整感觉切片、accessibility-size 结构切片与远端 CI 重建）；2026-08-10（最新远端 H015 收据与当前本地复测） |
| 对应 Git | `5dddd83602d0922bf78069dd6fdeaa5e5b5a5a5a`（内部 Host 基线；远端 CI run `31295533318` 已成功）；`79f1f36`（候选 3D 运行时防护）；`ea85c68`（证据提交；远端 CI run `31301067572` 已成功）；`9fc54d4`（文字部位 Area-only 修复；run `31303340128` 已成功）；`0105cd7`（完整感觉与感觉修订安全边界；run `31307042209` 已成功）；`7d8bf59`（P0 accessibility-size 结构优化；run `31311301360` 仅 HOST-T-015 failed）；`7e64833`（首个 HOST-T-015 自动化稳定性修复；run `31312416395` 仍仅 HOST-T-015 failed）；`e0f1089`（文字部位 Sheet 关闭后再断言 `body-map.next`；pushed SHA `306f2b91` 的 run `31313515662` 仍仅 HOST-T-015 failed）；`0ed0563`（第三次测试专用修复）；SHA `98a4200` / run `31314859140` 与 SHA `544c39a` / run `31318855810` 均为历史 H015 失败；`cb5f798` / run `31321251024` 为最新 terminal failure；本次未提交工作树的本地收据和远端 Pending 状态见下 |
...
> **2026-08-10 当前 H015 收据。** 最新远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功；Host smoke 12/1/0，唯一 H015 未在 5 秒内满足旧 `body-map.marker-count-summary` label CONTAINS `位置标记数量 1`，不能归因位置丢失。当前工作树修复为 active/empty count ID 在真实 count pill 上互斥呈现，Host 仅检查 stable ID 存在；CTA 保持 any-element `exists`/`isHittable`/`tap` 作为位置行为证明。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（138.685s），完整 internal Host script exit 0（441.966s）；本次修复已在本地完成、待提交推送与远端重试。它不改变真机、VoiceOver、完整 Dynamic Type、Reduce Motion、3D 性能/碰撞或资产审核的 Pending/Blocked 状态。旧的 136.2s/421.656s 数值均为历史本地收据。
...
7. `7d8bf59` 在本地 iPhone 17 Pro Max / iOS 26.5 完成 13 项 UI 流程与 94 项 Swift Core：HOST-T-015 只在 `UICTContentSizeCategoryAccessibilityXXXL` 下验证文字部位 Sheet、今天页两张展示性情境卡的稳定 ID/纵向分支和结构化页成对动作的局部结构。前三次远端失败历史保持不变；SHA `98a4200` 的 run `31314859140` 是第四次 H015 失败，文字部位 Sheet 关闭后 CTA 存在但不可点击；SHA `544c39a` / run `31318855810` 是历史 CTA failure。最新 `cb5f798` / run `31321251024` terminal failed：Host smoke 12/1/0，唯一 H015 未在 5 秒内满足旧 `body-map.marker-count-summary` label CONTAINS `位置标记数量 1`，不能归因位置丢失。当前工作树使 active/empty count ID 在真实 count pill 上互斥可见/可访问，Host 仅检查 stable ID 存在；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap`。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 为 2/0/0（138.685s），完整 internal Host script exit 0（441.966s）；本次修复已在本地完成，待提交推送与远端重试。它不构成 VoiceOver、完整最大 Dynamic Type、深色/高对比度、Reduce Motion、真实设备、碰撞、性能或资产审核证据。

因此，Sheet、VoiceOver、Dynamic Type、Reduce Motion、3D 性能、RealityKit 碰撞命中和候选资产的生产批准均保持 **Pending/Blocked**。`31318855810` 是历史 H015 CTA failure；最新远端 `31321251024` 的 H015 为旧 marker summary label predicate failure。当前工作树的摘要存在检查与 CTA 行为复测仅在 iPhone 17 Pro / iOS 26.5 形成 targeted H015 + MoreSensations 2/0/0（138.685s）和完整 internal Host script exit 0（441.966s）的内部 Simulator 收据；本次修复已在本地完成、待提交推送与远端重试，且不关闭上述任一设备门禁。2D、部位列表和安全入口继续是唯一可外推的可用路径；候选 USDZ 继续为 `candidate`，不得发布。
...
| GitHub Actions run `31321251024` | SHA `cb5f798`：Backend and contracts、Swift、iPhoneOS SDK build 与 Host boundary 成功；internal Host smoke 12 passed、1 failed、0 skipped；唯一 H015 在 5 秒内未满足旧 `body-map.marker-count-summary` label CONTAINS `位置标记数量 1` | 最新 terminal failure；summary element 的 stable-ID 存在并非失败对象，不能归因位置丢失；不证明无障碍、真机或发布通过 |
...
| SHA `544c39a` 前的 H015 本地收据 | 先前 safe-area CTA/identifier/Today 内容滚动/type-agnostic 查询及展示卡非动作断言后的 iPhone 17 Pro / iOS 26.5：定向 HOST-T-015 1 passed、0 failed、0 skipped（86.0s）；完整 `BODY_COMPANION_SIMULATOR_UDID=68F37251-71BE-4F42-9849-62D61BFFE7C3 bash scripts/run_internal_ios_host_tests.sh` exit 0（402.313s；脚本安静输出，不记录精确 XCTest 数） | 仅历史本地内部 Simulator 收据；不证明 SHA `544c39a` 的远端结果、VoiceOver、完整 Dynamic Type、真机或资产质量。后续 active/empty count 与 CTA 行为修复已在 iPhone 17 Pro / iOS 26.5 通过 targeted H015 + MoreSensations 2/0/0（138.685s）及完整 internal Host script exit 0（441.966s）；本次修复已在本地完成，待提交推送与远端重试 |
...
| Dynamic Type | **Historical Simulator structural smoke / device blocked** | 使用 `.body/.headline/.caption/.footnote` 等语义字体；`7d8bf59` 在 `UICTContentSizeCategoryAccessibilityXXXL` 下验证指定文字部位 Sheet、两张展示性情境卡和两组结构化成对动作的局部结构。`31314859140` 和 `31318855810` 均为历史 H015 CTA failure。最新 `31321251024`（SHA `cb5f798`）中 H015 在 5 秒内未满足旧 marker summary label predicate（12/1/0）；不能归因选择状态。当前修复保持 active/empty count ID 在真实 count pill 上互斥可见/可访问，Host 仅检查 stable ID 存在；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap`。iPhone 17 Pro / iOS 26.5 本地 targeted H015 + MoreSensations 2/0/0（138.685s）、完整 Host script exit 0（441.966s）；本次修复已在本地完成、待提交推送与远端重试 | 最大可访问字号和横屏下不截断标题、位置摘要、提示、结构化程度控件和按钮；确认 Sheet 可滚动且关键操作仍可达 |
...
| DEVICE-FINDING-007 | P2 | 2026-08-09 已从 `MarkEditor` 移除程度 Slider；结构化 `SignalIntakeScreen` 的程度控件虽有可见“程度”标签与 accessibility value，仍未经过真机 VoiceOver/最大 Dynamic Type 验证。`31314859140` 与 `31318855810` 均为历史 H015 CTA failure。最新 `31321251024`（SHA `cb5f798`）中 H015 为旧 marker summary label predicate failure（12/1/0），不能归因选择状态。当前修复使 active/empty count ID 在真实 count pill 上互斥可见/可访问，Host 仅检查 stable ID 存在；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap`。iPhone 17 Pro / iOS 26.5 本地 targeted H015 + MoreSensations 2/0/0（138.685s）及完整 Host script exit 0（441.966s）；本次修复已在本地完成、待提交推送与远端重试。位置摘要、完整页面、深色/高对比度和横屏仍未经过设备验证。 | 真机 VoiceOver/Dynamic Type 验证结构化程度控件；必要时取消摘要硬截断，并保留部位列表等价路径。 |
...
本轮决定：**记录历史基础 `Simulator host smoke passed`、`79f1f36` 的本地候选 loader lifecycle probe passed、`ea85c68` / run `31301067572` 的远端完整 Host smoke passed，以及 `9fc54d4` 的本地文字部位 Area-only Simulator flow 与 run `31303340128` 的远端 Simulator 重建成功；另记录 `0105cd7` 的本地完整感觉/感觉修订安全边界 Simulator flow 及 run `31307042209` 的远端重建成功。SHA `98a4200` / run `31314859140` 和 SHA `544c39a` / run `31318855810` 均为历史 H015 CTA failure。最新远端 `cb5f798` / run `31321251024` terminal failed：Host smoke 12/1/0，H015 未在 5 秒内满足旧 marker summary label predicate，不能归因选择丢失。当前工作树使 active/empty count ID 在真实 count pill 上互斥可见/可访问，Host 只检查 stable ID 存在；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap`。iPhone 17 Pro / iOS 26.5 上 targeted H015 + MoreSensations 为 2/0/0（138.685s），完整 Host script exit 0（441.966s）；本次修复已在本地完成、待提交推送与远端重试。** 不批准生产 3D，不声称真机通过或无障碍通过，不关闭 GATE-06/GATE-07/GATE-08。设备和正式 App target 到位前，生产路径保持 2D/列表 fail-closed；候选资产和 3D loader 仅限内部 prototype flag。
