...
| 快照日期 | 2026-08-09 |
| 对应 Git | `codex/initial-git-ci-baseline` 受保护默认分支；PR #4 合并提交 `2bb8e8a`；复刻切片提交 `6f232b4`、证据绑定提交 `f6a6d5b`；P0 体验/边界提交 `e93fc75`、GATE-02 决策包 `a91f103`、内部 Host `5dddd83`、候选 3D 运行时防护 `79f1f36`、候选 probe 证据提交 `ea85c68`、文字部位 Area-only 修复 `9fc54d4`、完整感觉与感觉修订安全边界 `0105cd7`、P0 accessibility-size 结构优化 `7d8bf59`、三次已提交的 HOST-T-015 自动化修复 `7e64833` / `e0f1089` / `0ed0563`；`0ed0563` 的远端 retry 尚未推送 |
| 本轮变更状态 | `7d8bf59` 已在本地完成 Swift Core、iPhoneOS SDK、静态 Host 边界与 13 项 Internal Host Simulator 验证；其中 HOST-T-015 仅覆盖 accessibility-size 下的局部结构/可达性路径。远端 CI run `31311301360`、`31312416395` 与 `31313515662` 都仅 HOST-T-015 失败（exit 65）。第三次为 pushed SHA `306f2b91` 的 iPhone 16 / iOS Simulator 18.5 / Xcode 16.4 运行，internal Host smoke 为 12 passed、1 failed、0 skipped，断言未找到静态文案“已添加待确认位置：左膝附近”；三个 run 的 Backend and contracts、Swift 94、iPhoneOS SDK build 与 Host 静态边界均成功。测试专用修复 `0ed0563` 不改变产品、健康或无障碍验收范围：它不再依赖短暂的本地化选择反馈，先验证无并存 `MarkEditor`、完成/关闭文字部位 Sheet 后再检查 `body-map.next`，并同样加固 `selectBoth`；本地 iPhone 17 Pro / iOS 26.5 的完整 Internal Host suite、单独 HOST-T-015 与 Swift Core 94/94 通过，远端 retry 尚未推送。历史 `0105cd7` 的 run `31307042209` 已成功完成 Backend and contracts 与 iOS package and internal host（Swift 94、iPhoneOS SDK build、Host boundary、internal Host smoke）。所有远端 iOS 结果均只形成 Simulator 证据，不构成真机或生产批准 |
...
| EV-CURRENT-017 | P0 accessibility-size 结构回归 | `7d8bf59` 在本地 iPhone 17 Pro Max / iOS 26.5 完成 13 项 Internal Host UI 流程、94 项 Swift Core、iPhoneOS SDK build 和 Host 静态检查。HOST-T-015 以 `UICTContentSizeCategoryAccessibilityXXXL` 只验证合成草稿后的 2D/文字部位 Sheet 选择、今天页两张情境提示卡纵向，以及结构化页“确认/未知”“保存/未知”成对动作纵向且可达。远端 run `31311301360`、`31312416395` 与 `31313515662` 均只有 HOST-T-015 失败（exit 65）；第三次为 pushed SHA `306f2b91` 的 iPhone 16 / iOS 18.5 / Xcode 16.4 运行，12 passed、1 failed、0 skipped，未找到静态文案“已添加待确认位置：左膝附近”。测试专用修复 `0ed0563` 改为验证无并存 `MarkEditor`、完成/关闭文字部位 Sheet 后再检查 `body-map.next`，并同步加固 `selectBoth`；本地 iPhone 17 Pro / iOS 26.5 的完整 Host suite、单独 HOST-T-015 与 Swift Core 94/94 通过，远端 retry 尚未推送 | 只证明当前本地 Simulator 的局部 a11y-size 结构与三次失败后的本地复测；不证明远端 retry、VoiceOver、真实最大 Dynamic Type、横屏、Switch Control、实际深色/高对比度、Reduce Motion、真机、3D 性能/碰撞、资产许可、签名或发布 |
...
- `7d8bf59` 为内部 Simulator 增加了 accessibility-size 下的结构回归；`31311301360`、`31312416395` 和 `31313515662` 都只失败于 HOST-T-015（exit 65），不应被隐去或写成远端通过。第三次仅缺少短暂的本地化选择反馈静态文案；测试专用修复 `0ed0563` 已在本地 iPhone 17 Pro / iOS 26.5 通过完整 Host suite、单独 HOST-T-015 与 Swift Core 94/94，远端 retry 尚未推送。所有记录都只证明指定合成路径的结构化自动化，不得写成完整 Dynamic Type、VoiceOver、深色/高对比度、Reduce Motion 或真机通过。
...
`7d8bf59` 在本地 iPhone 17 Pro Max / iOS 26.5 完成 13 项 Internal Host UI 流程、94 项 Swift Core、iPhoneOS SDK build 与 `check_internal_ios_host.py`。HOST-T-015 用 `UICTContentSizeCategoryAccessibilityXXXL` 走合成草稿：验证 2D/文字部位 Sheet 的选择、今天页两张情境提示卡的纵向布局，以及结构化描述页“确认/未知”“保存/未知”的纵向且可达布局。远端 run `31311301360`、`31312416395` 和 pushed SHA `306f2b91` 的 `31313515662` 均只失败于 HOST-T-015（exit 65）；第三次在 iPhone 16 / iOS Simulator 18.5 / Xcode 16.4 为 12 passed、1 failed、0 skipped，精确失败为未找到静态文案“已添加待确认位置：左膝附近”。测试专用修复 `0ed0563` 不再断言该短暂本地化反馈，而是先确认无并存 `MarkEditor`、完成/关闭文字部位 Sheet 后检查 `body-map.next`，并同样加固 `selectBoth`；本地 iPhone 17 Pro / iOS 26.5 的完整 Host suite、单独 HOST-T-015 与 Swift Core 94/94 已通过，远端 retry 尚未推送。它没有查询或操作位置摘要、焦点/重置、3D 回退提示，也没有验证 VoiceOver、完整最大字号、横屏、Switch Control、实际深色/高对比度、Reduce Motion、真机、性能、碰撞、资产许可或签名。
