...
| 快照日期 | 2026-08-09 |
| 对应 Git | `codex/initial-git-ci-baseline` 受保护默认分支；PR #4 合并提交 `2bb8e8a`；复刻切片提交 `6f232b4`、证据绑定提交 `f6a6d5b`；P0 体验/边界提交 `e93fc75`、GATE-02 决策包 `a91f103`、内部 Host `5dddd83`、候选 3D 运行时防护 `79f1f36`、候选 probe 证据提交 `ea85c68`、文字部位 Area-only 修复 `9fc54d4`、完整感觉与感觉修订安全边界 `0105cd7`、P0 accessibility-size 结构优化 `7d8bf59`、三次已提交的 HOST-T-015 自动化修复 `7e64833` / `e0f1089` / `0ed0563`；SHA `98a4200` / run `31314859140` 为第四次历史失败；SHA `544c39a` / run `31318855810` 为最新失败；当前 H015 修复和本地验证已完成，远端尚未推送 |
| 本轮变更状态 | `7d8bf59` 已在本地完成 Swift Core、iPhoneOS SDK、静态 Host 边界与 13 项 Internal Host Simulator 验证；HOST-T-015 仅覆盖 accessibility-size 下的局部结构/可达性路径。SHA `544c39a` 前的本地定向 HOST-T-015 1/0/0（86.0s）及完整 Host script exit 0（402.313s）仅为历史收据。最新远端 run `31318855810`（SHA `544c39a`）中 Backend and contracts 成功，iOS 仅 internal Host smoke 失败：12 passed、1 failed、0 skipped，HOST-T-015 报 `Expected element to exist: body-map.next Button`（exit 65）。CI 未提供 artifact/accessibility hierarchy，不能断定文字选择未保留位置，或 `body-map.next` 仅以不同 AX 投影/元素类型出现。当前工作树已实现 sibling footer 的底部独立、非滚动安全内容布局区；文字选择后先验证 `body-map.marker-count-summary` 表达恰有一项位置，随后以 generic stable-ID any-element 查询验证 CTA 存在、`isHittable` 且可 `tap`；不把具体 `safeAreaInset` API 或 `Button` class 作为跨 OS 契约。当前本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2 passed、0 failed、0 skipped（138.1s），完整 internal Host script exit 0（426.285s）。远端尚未推送，远端重试 Pending。历史 `0105cd7` 的 run `31307042209` 已成功完成 Backend and contracts 与 iOS package and internal host（Swift 94、iPhoneOS SDK build、Host boundary、internal Host smoke）。所有远端 iOS 结果均只形成 Simulator 证据，不构成真机或生产批准 |
...
| EV-CURRENT-009 | `.github/workflows/ci.yml` | Ubuntu 24.04 后端/契约 + macOS 15 iOS；官方 Action 固定完整 SHA；iOS job 新增 Host 静态扫描与 Simulator smoke | `5dddd83`、`ea85c68`、`9fc54d4` 与 `0105cd7` 的历史成功 CI 收据保留。历史第四次失败为 `31314859140`（SHA `98a4200`）：仅 HOST-T-015，文字部位 Sheet 关闭后 CTA 存在但不可点击。最新 `31318855810`（SHA `544c39a`）中 Backend and contracts 成功；iOS 仅 internal Host smoke 失败（12 passed、1 failed、0 skipped），HOST-T-015 报 `Expected element to exist: body-map.next Button`（exit 65）。CI 无 artifact/accessibility hierarchy，不能归因。SHA `544c39a` 前的 1/0/0（86.0s）和 full Host script exit 0（402.313s）仅是历史本地收据。当前 H015 的 sibling footer、marker-count 一项位置先验与 generic stable-ID any-element CTA exists/hittable/tap 已实现，不以 `safeAreaInset` 或 `Button` class 为条件；iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送、远端重试 Pending |
...
| EV-CURRENT-017 | P0 accessibility-size 结构回归 | `7d8bf59` 在本地 iPhone 17 Pro Max / iOS 26.5 完成 13 项 Internal Host UI 流程、94 项 Swift Core、iPhoneOS SDK build 和 Host 静态检查。HOST-T-015 以 `UICTContentSizeCategoryAccessibilityXXXL` 只验证合成草稿后的局部结构。历史 SHA `98a4200` / run `31314859140` 为 Sheet 关闭后 CTA 存在但不可点击；SHA `544c39a` 前的定向 1/0/0（86.0s）和 full Host script exit 0（402.313s）只为历史本地收据。最新 run `31318855810`（SHA `544c39a`）中 Backend and contracts 成功，iOS 仅 Host smoke 失败（12 passed、1 failed、0 skipped），HOST-T-015 报 `Expected element to exist: body-map.next Button`（exit 65）。因 CI 无 artifact/accessibility hierarchy，不能把它判定为选择丢失或 AX 投影/元素类型。当前 sibling footer、marker-count summary 一项位置断言与 generic stable-ID any-element CTA exists/hittable/tap 已实现；不指定 `safeAreaInset` 或 `Button` class。iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送，远端重试 Pending | 只证明当前本地 Simulator 的局部 a11y-size 自动化；不证明远端通过、VoiceOver、真实最大 Dynamic Type、横屏、Switch Control、实际深色/高对比度、Reduce Motion、真机、3D 性能/碰撞、资产许可、签名或发布 |
...
- `7d8bf59` 为内部 Simulator 增加了 accessibility-size 下的结构回归；历史 SHA `98a4200` / run `31314859140` 的 H015 在 Sheet 关闭后 CTA 存在但不可点击，不应被隐去。SHA `544c39a` 前的本地定向 1/0/0（86.0s）和完整 Host script exit 0（402.313s）也只能作为历史收据。最新 run `31318855810`（SHA `544c39a`）中 Backend and contracts 成功，但 iOS 仅 Host smoke 失败（12 passed、1 failed、0 skipped），错误为 `Expected element to exist: body-map.next Button`（exit 65）；CI 无 artifact/accessibility hierarchy，根因未定。当前实现已落地：sibling footer 的底部独立、非滚动安全内容布局、marker-count summary 一项位置先验和 generic stable-ID any-element CTA exists/hittable/tap，而非 `safeAreaInset` 或 `Button` class 契约。本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送，远端重试 Pending。所有记录都只证明指定合成路径的结构化自动化，不得写成完整 Dynamic Type、VoiceOver、深色/高对比度、Reduce Motion 或真机通过。
...
`7d8bf59` 在本地 iPhone 17 Pro Max / iOS 26.5 完成 13 项 Internal Host UI 流程、94 项 Swift Core、iPhoneOS SDK build 与 `check_internal_ios_host.py`。HOST-T-015 用 `UICTContentSizeCategoryAccessibilityXXXL` 走合成草稿：验证 2D/文字部位 Sheet 的选择、今天页两张展示性情境卡的稳定 ID/纵向分支，以及结构化描述页“确认/未知”“保存/未知”的局部动作结构。历史 SHA `98a4200` / run `31314859140` 仍是 H015 的 CTA 存在但不可点击失败。SHA `544c39a` 前的定向 1/0/0（86.0s）和完整 Host script exit 0（402.313s）仅为历史本地收据。最新 SHA `544c39a` / run `31318855810`：Backend and contracts 成功，iOS 仅 Host smoke 失败（12 passed、1 failed、0 skipped）；HOST-T-015 报 `Expected element to exist: body-map.next Button`（exit 65）。CI 无 artifact/accessibility hierarchy，无法决定位置选择未保留还是 AX 投影/元素类型。当前修复已实现：sibling footer 的底部独立、非滚动安全内容区；文字选择后先验证 `body-map.marker-count-summary` 的一项位置，接着 generic stable-ID any-element CTA exists/hittable/tap；不以 `safeAreaInset` 或 `Button` class 为跨 OS 条件。本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送，远端 retry Pending。它没有查询或操作位置摘要、焦点/重置、3D 回退提示，也没有验证 VoiceOver、完整最大字号、横屏、Switch Control、实际深色/高对比度、Reduce Motion、真机、性能、碰撞、资产许可或签名。
