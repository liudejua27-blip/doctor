...
| 快照日期 | 2026-08-10 |
| 对应 Git | `codex/initial-git-ci-baseline` 受保护默认分支；PR #4 合并提交 `2bb8e8a`；复刻切片提交 `6f232b4`、证据绑定提交 `f6a6d5b`；P0 体验/边界提交 `e93fc75`、GATE-02 决策包 `a91f103`、内部 Host `5dddd83`、候选 3D 运行时防护 `79f1f36`、候选 probe 证据提交 `ea85c68`、文字部位 Area-only 修复 `9fc54d4`、完整感觉与感觉修订安全边界 `0105cd7`、P0 accessibility-size 结构优化 `7d8bf59`、三次已提交的 HOST-T-015 自动化修复 `7e64833` / `e0f1089` / `0ed0563`；SHA `98a4200` / run `31314859140` 与 SHA `544c39a` / run `31318855810` 均为历史失败；`cb5f798` / run `31321251024` 为最新 terminal failure；当前 H015 修复和本地验证已完成，远端尚未推送 |
| 本轮变更状态 | `7d8bf59` 的 accessibility-size 局部结构历史收据保留。SHA `544c39a` / run `31318855810` 为历史 H015 失败。最新远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，internal Host smoke 12/1/0；唯一 H015 在 5 秒内未满足 `body-map.marker-count-summary` label CONTAINS `位置标记数量 1`，不能归因位置丢失。当前工作树使摘要在真实 count pill 上保持可见/可访问，Host 对摘要只检查 stable identifier 存在；CTA 再以 generic stable-ID any-element 验证 `exists`、`isHittable` 与 `tap`。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端重试 Pending。所有远端 iOS 结果均只形成 Simulator 证据，不构成真机或生产批准 |
| **2026-08-10 当前 H015 收据** | 最新远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功；internal Host smoke 12 passed、1 failed、0 skipped，唯一 H015 在 5 秒内未满足 `body-map.marker-count-summary` label CONTAINS `位置标记数量 1`，不能归因位置丢失。当前工作树使摘要保持可见/可访问、stable ID 位于真实 count pill，Host 对摘要只检查存在；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap` 作为位置已保留的行为证明。本地 iPhone 17 Pro / iOS 26.5：targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复尚未推送、远端重试 Pending | 仅当前本地内部 Simulator 收据；不证明远端通过、真机、VoiceOver、完整 Dynamic Type、Reduce Motion、性能、碰撞、资产、签名或发布 |
...
| EV-CURRENT-009 | `.github/workflows/ci.yml` | Ubuntu 24.04 后端/契约 + macOS 15 iOS；官方 Action 固定完整 SHA；iOS job 新增 Host 静态扫描与 Simulator smoke | 历史失败 `31314859140`（SHA `98a4200`）和 `31318855810`（SHA `544c39a`）保留。最新 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；H015 仅未在 5 秒内满足 marker summary label 断言，不能归因位置丢失。当前 H015 摘要在真实 count pill 上可见/可访问，Host 只检查其 identifier 存在；CTA 保持 any-element `exists`/`isHittable`/`tap`。iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端重试 Pending |
...
| EV-CURRENT-017 | P0 accessibility-size 结构回归 | `7d8bf59` 的局部 accessibility-size 结构历史收据保留。SHA `544c39a` / run `31318855810` 为历史 H015 失败。最新 `cb5f798` / run `31321251024` 的 Host smoke 为 12/1/0；H015 在 5 秒内未满足 marker summary label 断言，不能判定选择丢失。当前摘要在真实 count pill 上可见/可访问，Host 仅检查 identifier 存在；CTA 继续 any-element `exists`/`isHittable`/`tap`。iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端 Pending | 只证明当前本地 Simulator 的局部 a11y-size 自动化；不证明远端通过、VoiceOver、真实最大 Dynamic Type、横屏、Switch Control、实际深色/高对比度、Reduce Motion、真机、3D 性能/碰撞、资产许可、签名或发布 |
...
- `7d8bf59` 为内部 Simulator 增加了 accessibility-size 下的结构回归；历史 SHA `98a4200` / run `31314859140` 的 H015 在 Sheet 关闭后 CTA 存在但不可点击，SHA `544c39a` / run `31318855810` 是历史 CTA failure。最新远端 `cb5f798` / run `31321251024` terminal failed：Host smoke 12/1/0，H015 仅未在 5 秒内满足 `body-map.marker-count-summary` label 断言，不能归因位置丢失。当前工作树使摘要在真实 count pill 上保持可见/可访问，Host 只检查摘要 identifier 存在；CTA 继续 stable-id any-element `exists`/`isHittable`/`tap`。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 为 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端重试 Pending。所有记录都只证明指定合成路径的结构化自动化，不得写成完整 Dynamic Type、VoiceOver、深色/高对比度、Reduce Motion 或真机通过。
...
`7d8bf59` 在本地 iPhone 17 Pro Max / iOS 26.5 完成 13 项 Internal Host UI 流程、94 项 Swift Core、iPhoneOS SDK build 与 `check_internal_ios_host.py`。HOST-T-015 用 `UICTContentSizeCategoryAccessibilityXXXL` 走合成草稿：验证 2D/文字部位 Sheet 的选择、今天页两张展示性情境卡的稳定 ID/纵向分支，以及结构化描述页“确认/未知”“保存/未知”的局部动作结构。历史 SHA `98a4200` / run `31314859140` 是 CTA 存在但不可点击失败，SHA `544c39a` / run `31318855810` 是历史 CTA failure。最新远端 `cb5f798` / run `31321251024` 的 Host smoke 为 12/1/0：H015 仅未在 5 秒内满足 `body-map.marker-count-summary` label 断言，不能归因位置选择或 AX 根因。当前工作树保持摘要在真实 count pill 上可见/可访问，Host 仅检查摘要 identifier 存在；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap`。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 为 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端重试 Pending。它没有查询或操作位置摘要、焦点/重置、3D 回退提示，也没有验证 VoiceOver、完整最大字号、横屏、Switch Control、实际深色/高对比度、Reduce Motion、真机、性能、碰撞、资产许可或签名。
