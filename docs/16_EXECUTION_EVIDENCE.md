...
| 快照日期 | 2026-08-09 |
| 对应 Git | `codex/initial-git-ci-baseline` 受保护默认分支；PR #4 合并提交 `2bb8e8a`；复刻切片提交 `6f232b4`、证据绑定提交 `f6a6d5b`；P0 体验/边界提交 `e93fc75`、GATE-02 决策包 `a91f103`、内部 Host `5dddd83`、候选 3D 运行时防护 `79f1f36` |
| 本轮变更状态 | `79f1f36` 已在本地完成当前候选 3D 专用 probe；对应远端 CI 在推送前为 Pending。所有 iOS Host 结论仅限 Simulator，不构成真机或生产批准 |
...
历史 iOS Host 基线绑定提交 `5dddd83`：本地 iPhone 17 Pro Simulator 与远端 CI iPhone 16 Simulator 均已重建。当前候选 3D 运行时防护绑定 `79f1f36`，已获得本地专用 probe 证据，远端结果仍待本次推送后的 CI。进入外测前仍必须由受保护提交、SafetyBaseline、真实设备和所有发布门禁另行验证。

## 2. 当前验证结果

| ID | 检查 | 当前结果 | 证明范围 |
|---|---|---|---|
| EV-CURRENT-001 | `scripts/check_baseline.py` | `passed; markdown_files=62 checked_links=361 json_schemas=16 openapi_paths=36 openapi_schemas=111 asset_manifests=1 prohibited_source_matches=0` | 文档、契约清单、CI Action SHA/权限、依赖 pin、资产清单/Bundle SHA、退役路径和禁止源码引用的本地静态门禁 |
...
- 受版本控制的 `BodyCompanionInternal` 已可在本地和远端 Simulator 构建、安装、启动并通过历史 7 项基础 UI smoke；`79f1f36` 本地 9 项套件另观察到候选 3D 的 loader-entry → candidate-ready → 场景/列表路径。候选开关、网络/Provider、权限、持久化、遥测和正式档案能力均显式保持关闭；当前候选 probe 的远端 CI 仍待执行。
...
`79f1f36` 在本地 iPhone 17 Pro / iOS 26.5 完成 9 项内部 Host UI 测试：专用 probe 只设置候选开关，先读到当前 Scene 的 loader-entry，再实际进入 candidate-ready 并保留 3D 场景与部位列表；未点击网格，未生成 `BodyLocation` 或其他健康事实。当前远端 CI 尚待本次推送；无论本地或远端结果，都不构成候选资产、碰撞、性能、无障碍、签名、真机或生产批准。
