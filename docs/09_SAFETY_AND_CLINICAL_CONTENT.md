...
| 适用地区 | 中国大陆、App Store |
| 变更级别 | A |
| 依赖 | DOC-00、TERM-01、PROD-01、AGENT-01、PRIV-01、QA-01、ADR-0002、ADR-0018 |
...
stateDiagram-v2
    ...
    DeterministicRules --> SupportedNormalFlow: R2 或 R3 且场景已审核
...
| R2 / `PromptProfessionalReview` | 当前信息适合尽快预约专业评估 | 说明不确定性；提供审核过的预约准备、等待期间记录方式与升级条件 | 以短期观察或自我管理替代专业评估；宣称安全、排除疾病、给出诊断或个体治疗 |
...
仅在场景已审核、规则输入充分且输出通过策略验证时使用。可以包含事实摘要、非诊断性模式、审核行动内容、复查计划和升级条件。
...
- R2、R3 的一般内容也必须来自内容库；LLM 只能选择、排序和在允许的槽位中填入用户确认事实。
