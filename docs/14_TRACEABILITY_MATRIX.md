...
> **2026-08-10 HOST-T-015 当前收据。** 最新远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功；internal Host smoke 为 12 passed、1 failed、0 skipped，唯一 H015 在 5 秒内未满足 `body-map.marker-count-summary` label CONTAINS `位置标记数量 1`。这不能归因于位置丢失。当前工作树修复为摘要保持可见/可访问、stable ID 位于真实 count pill、Host 对摘要只检查存在；CTA 仍以 stable-id any-element 的 `exists`/`isHittable`/`tap` 作为位置已保留的行为证明。本地 iPhone 17 Pro / iOS 26.5：targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复尚未推送，远端重试 Pending。此收据仍仅限内部 Simulator，不外推为真机或完整无障碍通过。
...
|---|---|---|---|---|---|
| PRD-F01 30 秒结构化记录 | UX-01、IOS-01、DATA-01、CONFLICT-003、CONFLICT-004 | `ios-signal-intake`、`body-signal-event` | Swift typed intake 状态机；多位置感觉必须显式关联 Marker，新增位置不复制感觉；`0105cd7` 已由 94 项 Core/12 项 Host flow 覆盖完整稳定感觉目录和安全修订边界。历史远端失败保留：SHA `98a4200` / run `31314859140` 的 HOST-T-015 在 Sheet 关闭后 `body-map.next` 存在但不可点击，SHA `544c39a` / run `31318855810` 也是历史 H015 失败。最新 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，iOS Host smoke 12/1/0；唯一 H015 在 5 秒内未满足 `body-map.marker-count-summary` label CONTAINS `位置标记数量 1`，不能归因位置丢失。当前工作树使摘要在真实 count pill 上保持可见/可访问，Host 对摘要只检查 stable identifier 存在；CTA 再按 stable-id any-element 查询验证 `exists`、`isHittable` 和 `tap`，作为位置已保留的行为证明；具体 `safeAreaInset` API 与 `Button` class 均不是跨 OS 契约。本地 iPhone 17 Pro / iOS 26.5 收据：targeted HOST-T-015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）。新修复未推送，远端收据 Pending。位置及其他安全相关事实的高风险修订仍由 CONFLICT-004 阻断；当前会话草稿统一“继续/确认放弃后新建” | Prototype verified / P1D 感觉安全修订及局部 accessibility-size 结构已有历史证据；最新实现与本地收据已完成，远端与外部发布仍阻断 | GATE-03/06/07/08 |
...
| PRD-F01/F03A 内部运行宿主 | FEAT-IOS-P0-RUNTIME-01、IOS-01、PRIV-01、QA-01 | 无新增契约；沿用 `body-location`、`ios-signal-intake@1.1`、`ios-draft-envelope@1.1` | `5dddd83` 的 Xcode Host/7 项本地 UI smoke/run `31295533318` 远端 Simulator smoke 通过；`79f1f36`、`9fc54d4` 与 `0105cd7` 的历史本地/远端收据保留。SHA `98a4200` / run `31314859140` 和 SHA `544c39a` / run `31318855810` 均为历史 H015 失败。最新 `cb5f798` / run `31321251024`：Backend and contracts、Swift、SDK 与 Host boundary 成功，iOS Host smoke 12/1/0；H015 在 5 秒内未满足 marker summary 的 label 断言，不能判定选择丢失。当前工作树已实现 sibling footer、真实 count pill 上可见/可访问的摘要及 Host-only 摘要存在检查；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap`，不得把 `safeAreaInset` 或 `Button` class 设为跨 OS 条件。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端收据 Pending | Implemented internal engineering prototype / Simulator only；当前 H015 修复和本地验证已完成，远端未通过前不关闭门禁 | GATE-02/06/07/08 仍打开 |
...
| SAFE-INV-13 | 语义感觉修订不得复用旧安全、普通 Agent 或审批 | UX-01、DATA-01、SAFE-01、CONFLICT-003 | `0105cd7` 的 94 项 Core 回归覆盖普通本地安全状态下的原子失效、R0/R1/R2/`undetermined`/`safety_action` 拒绝、malformed action phase 与 no-op 不变性；12 项本地 Host 流程覆盖“更多感觉”结构化入口。run `31307042209` 已成功重建 Backend and contracts 与 iPhone 16 / iOS Simulator 18.5 iOS job | 旧安全/Agent/approval 复用 0；R0/R1/R2/undetermined 不得被本地清空 |
...
| NFR-A11Y-001 等价路径 | 2D/列表设计存在；`9fc54d4` 的文字部位 Sheet 与 `0105cd7` 的完整感觉入口均有 Simulator UI 流程覆盖；HOST-T-015 仅在 `UICTContentSizeCategoryAccessibilityXXXL` 下检查局部结构。SHA `544c39a` / run `31318855810` 是历史 H015 失败。最新 `cb5f798` / run `31321251024` 的 Backend and contracts、Swift、SDK 与 Host boundary 成功，但 Host smoke 为 12/1/0：H015 在 5 秒内未满足 marker summary label 断言，不能归因。当前 H015 使摘要在真实 count pill 上保持可见/可访问，Host 只检查摘要 identifier 存在；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap`。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端收据 Pending。EVIDENCE-DEVICE-01 仍记录真机执行阻塞 | VoiceOver、完整 Dynamic Type、横屏、Switch Control、深色/高对比度、Reduce Motion 真机 |
...
| NFR-RUNTIME-001 内部 Simulator 可运行性 | `5dddd83`、`79f1f36`、`9fc54d4`、`0105cd7` 和相应历史远端 run 的 Simulator 收据保留。SHA `98a4200` / run `31314859140` 与 SHA `544c39a` / run `31318855810` 均为历史 H015 失败。最新 `cb5f798` / run `31321251024`：Backend and contracts、Swift、SDK 与 Host boundary 成功，iOS Host smoke 12/1/0；H015 在 5 秒内未满足 marker summary label 断言，不能归因位置状态。当前布局/测试修复已实现：sibling footer 的独立非滚动安全内容区、真实 count pill 上可见/可访问的摘要及 Host-only 摘要存在检查；CTA 保持 stable-id any-element `exists`/`isHittable`/`tap`，不锁定 `safeAreaInset` 或 `Button` class。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端收据 Pending | 不得外推真机、签名、VoiceOver、完整 Dynamic Type、横屏、Switch Control、深色/高对比度、Reduce Motion、3D 性能/碰撞或无障碍通过 |
