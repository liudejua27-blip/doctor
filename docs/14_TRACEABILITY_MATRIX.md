...
|---|---|---|---|---|---|
| PRD-F01 30 秒结构化记录 | UX-01、IOS-01、DATA-01、CONFLICT-003、CONFLICT-004 | `ios-signal-intake`、`body-signal-event` | Swift typed intake 状态机；多位置感觉必须显式关联 Marker，新增位置不复制感觉；`0105cd7` 已由 94 项 Core/12 项 Host flow 覆盖完整稳定感觉目录和安全修订边界。历史远端失败保留：SHA `98a4200` / run `31314859140` 的 HOST-T-015 在 Sheet 关闭后 `body-map.next` 存在但不可点击。SHA `544c39a` 前的本地定向 1/0/0（86.0s）和完整 Host script exit 0（402.313s）仅为历史收据。最新 run `31318855810`（SHA `544c39a`）中 Backend and contracts 成功，iOS 仅 Host smoke 失败：12 passed、1 failed、0 skipped，HOST-T-015 报 `Expected element to exist: body-map.next Button`（exit 65）；CI 无 artifact/accessibility hierarchy，选择未保留位置与 AX 投影/元素类型无法归因。当前工作树已实现 sibling footer：底部独立、非滚动安全内容布局区；文字选择后先断言 `body-map.marker-count-summary` 恰有一项位置，再按 stable-id any-element 查询验证 CTA 存在、`isHittable` 和 `tap`；具体 `safeAreaInset` API 与 `Button` class 均不是跨 OS 契约。当前本地 iPhone 17 Pro / iOS 26.5 收据：targeted HOST-T-015 + MoreSensations 为 2 passed、0 failed、0 skipped（138.1s），完整 internal Host script exit 0（426.285s）。远端尚未推送，远端收据 Pending。位置及其他安全相关事实的高风险修订仍由 CONFLICT-004 阻断；当前会话草稿统一“继续/确认放弃后新建” | Prototype verified / P1D 感觉安全修订及局部 accessibility-size 结构已有历史证据；最新实现与本地收据已完成，远端与外部发布仍阻断 | GATE-03/06/07/08 |
...
| PRD-F01/F03A 内部运行宿主 | FEAT-IOS-P0-RUNTIME-01、IOS-01、PRIV-01、QA-01 | 无新增契约；沿用 `body-location`、`ios-signal-intake@1.1`、`ios-draft-envelope@1.1` | `5dddd83` 的 Xcode Host/7 项本地 UI smoke/run `31295533318` 远端 Simulator smoke 通过；`79f1f36`、`9fc54d4` 与 `0105cd7` 的历史本地/远端收据保留。历史 SHA `98a4200` / run `31314859140` 的 HOST-T-015 仍为 Sheet 关闭后 CTA 存在但不可点击。最新 SHA `544c39a` / run `31318855810`：Backend and contracts 成功，iOS 仅 Host smoke 失败（12 passed、1 failed、0 skipped）；错误为 `Expected element to exist: body-map.next Button`（exit 65）。CI 无 artifact/accessibility hierarchy，不能判定选择丢失或 AX 投影/元素类型。SHA `544c39a` 前的本地 1/0/0（86.0s）及 full Host script exit 0（402.313s）只作历史记录。当前工作树已实现 sibling footer、marker-count 一项位置先验及 stable-id any-element CTA `exists`/`isHittable`/`tap`；不得把 `safeAreaInset` 或 `Button` class 设为跨 OS 条件。当前本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送，远端收据 Pending | Implemented internal engineering prototype / Simulator only；当前 H015 修复和本地验证已完成，远端未通过前不关闭门禁 | GATE-02/06/07/08 仍打开 |
...
| NFR-A11Y-001 等价路径 | 2D/列表设计存在；`9fc54d4` 的文字部位 Sheet 与 `0105cd7` 的完整感觉入口均有 Simulator UI 流程覆盖；HOST-T-015 仅在 `UICTContentSizeCategoryAccessibilityXXXL` 下检查局部结构。SHA `544c39a` 前的定向 1/0/0（86.0s）与 full Host script exit 0（402.313s）是历史本地证据。最新 run `31318855810`（SHA `544c39a`）的 Backend and contracts 成功，但 iOS Host smoke 以 `Expected element to exist: body-map.next Button` 失败（12 passed、1 failed、0 skipped，exit 65）；CI 未提供 artifact/hierarchy，不能归因。当前 H015 已实现 sibling footer、marker-count summary 一项位置断言与 stable-id any-element CTA exists/hittable/tap；不以 `safeAreaInset` 或 `Button` class 为条件。当前本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送，远端收据 Pending。EVIDENCE-DEVICE-01 仍记录真机执行阻塞 | VoiceOver、完整 Dynamic Type、横屏、Switch Control、深色/高对比度、Reduce Motion 真机 |
...
| NFR-RUNTIME-001 内部 Simulator 可运行性 | `5dddd83`、`79f1f36`、`9fc54d4`、`0105cd7` 和相应历史远端 run 的 Simulator 收据保留。历史 SHA `98a4200` / run `31314859140` 的 H015 是 CTA 存在但不可点击。最新 SHA `544c39a` / run `31318855810`：Backend and contracts 成功，iOS 仅 Host smoke failed（12 passed、1 failed、0 skipped），`Expected element to exist: body-map.next Button`（exit 65）；CI 无 artifact/accessibility hierarchy，不能确定选择状态还是 AX 投影/元素类型。SHA `544c39a` 前的定向 1/0/0（86.0s）和完整 Host script exit 0（402.313s）仅历史。当前布局/测试修复已实现：sibling footer 的独立非滚动安全内容区、marker-count 一项位置先验与 stable-id any-element CTA exists/hittable/tap；不锁定 `safeAreaInset` 或 `Button` class。当前本地 iPhone 17 Pro / iOS 26.5 的 targeted HOST-T-015 + MoreSensations 为 2/0/0（138.1s），完整 internal Host script exit 0（426.285s）；远端尚未推送，远端收据 Pending | 不得外推真机、签名、VoiceOver、完整 Dynamic Type、横屏、Switch Control、深色/高对比度、Reduce Motion、3D 性能/碰撞或无障碍通过 |
