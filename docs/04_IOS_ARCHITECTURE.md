...
`BodyMapScreen` 负责一个纯表现层的继续动作规则：只要当前未确认地图至少有一个位置，稳定 identifier `body-map.next` 必须在页面底部 safe-area 的独立操作区（SwiftUI `safeAreaInset` 或等价原生机制）保持可见、可聚焦和可触发。主内容滚动区必须保留同等底部空间；文字部位 Sheet 的 `body-map.text-picker-done` 关闭后，该动作不依赖通用滚动才可操作。该规则不新增页面状态、健康字段、`BodyLocation` 映射、API、网络、持久化或安全/AI 行为；无位置时不呈现继续动作。
...
- 当地图已有至少一个未确认位置时，`body-map.next` 必须位于底部 safe-area 的独立操作区，滚动内容不得覆盖该区；文字部位 Sheet 完成/关闭后，该主动作必须立即可聚焦和触发，而不是要求用户以额外滚动寻找它；
...
- Internal Host 的 accessibility-size 结构 smoke；在文字部位 Sheet 完成/关闭后，它还检查 `body-map.next` 作为底部 safe-area 主动作立即可达，而不是用通用滚动掩盖布局缺陷；它不能取代设备上的 VoiceOver、最大字号、横屏、深色/高对比度或 Switch Control 验收；
- 用户修改和确认后服务端回读一致；
- 数据导出和删除端到端。
