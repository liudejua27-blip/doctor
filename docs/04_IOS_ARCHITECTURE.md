...
`BodyMapScreen` 负责一个纯表现层的继续布局规则：页面底部独立、非滚动的安全内容布局区始终存在。没有位置时，该区仅呈现非动作说明 `body-map.continuation-unavailable`，不得调用路由；至少一个未确认位置保留后，既有稳定动作 identifier `body-map.next` 才呈现、可聚焦并调用由 `AppShell` 所有的 UI-only continuation closure，把既有 `RouterPath` 追加到结构化描述路由。不可将 `body-map.next` 复用为无位置说明。SwiftUI `safeAreaInset` 只是可选实现，不是 API 契约；任何等价原生布局机制都必须让该区不随主内容滚动，并让主内容为它预留同等底部空间。文字部位 Sheet 的 `body-map.text-picker-done` 关闭后，`body-map.pending-mark-summary` 必须是实际待确认位置摘要；文字选择只清除视觉 selectedMark，不得删除位置或自动弹出 Marker editor。实际 count pill 的 identifier 也必须区分会话状态：无位置时只显示非动作 `body-map.marker-count-empty`，至少一个 provisional `BodyLocation` 保留后才显示 `body-map.marker-count-summary`；两者互斥，精确数量/去重由 Core 回归锁定。Host 先验证 Sheet 内稳定、当前可见的文字选择反馈和 pending summary，再按 stable identifier 的任意元素查询验证 active count 存在且 empty count 不存在，随后验证 `body-map.next` 已启用、`isHittable` 且可 `tap`；无结果或清空最后位置只在 Sheet 关闭后验证相反的 count 状态。该条件主动作是至少一个未确认位置已保留的黑箱证明，且不得把 AX element class（尤其 `Button`）作为跨 OS 契约。该规则不新增健康字段、`BodyLocation` 映射、API、网络、持久化或安全/AI 行为。
...
- 底部继续布局区必须始终独立、非滚动，滚动内容不得覆盖该区；无位置时它呈现非动作说明 `body-map.continuation-unavailable`，只有已有至少一个未确认位置时才呈现并可触发既有动作 `body-map.next`。文字部位 Sheet 完成/关闭后，实际待确认位置摘要 `body-map.pending-mark-summary` 与 count pill 的互斥状态（有位置为 `body-map.marker-count-summary`，无位置为 `body-map.marker-count-empty`）必须保持可见、可访问；Host 先以 stable identifier 验证 active/empty count 互斥，再以 stable identifier 的任意元素查询验证主动作已启用、可点击并可触发，而不是要求用户以额外滚动寻找它；精确数量/去重由 Core 覆盖，SwiftUI `Text` AX label/value/精确文案、具体布局 API 与 AX element class 不是该 Host 验收的契约；
...
- Internal Host 的 accessibility-size 结构 smoke；Sheet 内稳定可见的文字选择反馈、Sheet 关闭后的 `body-map.pending-mark-summary`、实际 count pill 的互斥 active/empty identifier 与未自动弹出的 Marker editor 共同形成位置保留的用户可见闭环，再用 stable identifier 的任意元素查询检查 `body-map.next` 已启用、位于底部独立非滚动区域、可点击和可触发，而不是用通用滚动、SwiftUI `Text` label/value/精确文案或元素 class 掩盖布局/AX 投影缺陷；精确数量/去重由 Core 回归锁定。它不能取代设备上的 VoiceOver、最大字号、横屏、深色/高对比度或 Switch Control 验收；
- 用户修改和确认后服务端回读一致；
- 数据导出和删除端到端。
