...
P1D 的 SwiftUI 结构化描述页将本步骤拆为 `SignalSensationCode`、`SignalIntensity` 和显式复核状态；不预选“酸痛”，未填写与 0 分分开，候选和用户事实按来源展示。地图中的位置 Inspector 只显示/删除 Zone 或 Pin、部位、侧别、表面和精确点状态；它不出现感觉、程度或动作线索控件。实现与验收见 [FEAT-P1D](18_IMPLEMENTED_PROTOTYPE_BASELINE.md)。
