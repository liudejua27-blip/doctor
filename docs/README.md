...
- [FEAT-IOS-P0-RUNTIME-01](28_IOS_P0_RUNTIME_HOST.md) 的仅限内部 Simulator App Host 已在本地与远端 CI 通过；`0105cd7` 的 94 项 Core 与 HOST-T-001～014 已由 run `31307042209` 在 iPhone 16 / iOS Simulator 18.5 重建。该工程例外不新增产品样机层，也不替代真机/生产验证；接下来仍须按 [REL-01 当前执行顺序](13_DELIVERY_ROADMAP.md#3-当前执行顺序)关闭生产门禁。
...
- [FEAT-IOS-P0-RUNTIME-01 内部 iOS App Host](28_IOS_P0_RUNTIME_HOST.md)：受版本控制的 Simulator App Host、显式关闭能力集、默认 P0 UI smoke 与专用候选 3D probe；基础 smoke、候选 probe 与完整感觉入口均已在本地及 run `31307042209` 的远端 CI 重建，internal only。
- [TEST-IOS-P0-RUNTIME-01 内部 Host 测试计划](29_IOS_P0_RUNTIME_HOST_TEST_PLAN.md)：构建、安装、启动、2D/列表回退、无网络/权限/持久化负向检查、候选 Scene loader-entry/终态及完整感觉/unknown 入口；本地和远端 CI 均只形成 Simulator 证据。
...
| TEST-IOS-P0-RUNTIME-01 | [内部 Host 测试计划](29_IOS_P0_RUNTIME_HOST_TEST_PLAN.md) | Host 的 Simulator 构建、启动、P0 路径及负向能力检查 | Executed: HOST-T-001～014 / Simulator-only evidence |
