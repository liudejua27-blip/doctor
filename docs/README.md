...
| P0 | 远端版本门禁 | 将已建立的 `codex/initial-git-ci-baseline` 与 CI workflow 配置到远端；首次云端运行绿色，并把检查设为默认分支必需状态 | 提交 `b9e45d1` 已推送且远端默认分支暂为该分支；CI 运行 `31234199534` 两个 job 成功，分支保护 API 读回成功；真实 PR 合并路径仍未验证 |
