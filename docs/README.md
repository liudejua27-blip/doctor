...
- **当前 H015 收据（2026-08-10，以下较早 H015 收据均为历史）。** 远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；唯一 H015 未在 5 秒内满足 `body-map.marker-count-summary` 的旧计数文字断言，不能归因位置丢失。当前工作树使摘要保持可见/可访问、stable ID 位于真实 count pill，Host 对摘要只检查摘要 identifier 存在；CTA 仍以 any-element `exists`/`isHittable`/`tap` 作为位置已保留的行为证明。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（136.2s），完整 internal Host script exit 0（421.656s）；新修复未推送、远端重试 Pending。仅内部 Simulator，不构成真机或完整无障碍通过。
- [FEAT-IOS-P0-RUNTIME-01](28_IOS_P0_RUNTIME_HOST.md) 的仅限内部 Simulator App Host 已有本地与历史远端 CI 收据；`0105cd7` 的 94 项 Core 与 HOST-T-001～014 已由 run `31307042209` 在 iPhone 16 / iOS Simulator 18.5 重建。历史 SHA `98a4200` / run `31314859140` 的 H015 为文字部位 Sheet 关闭后 CTA 存在但不可点击；SHA `544c39a` 前的本地定向 H015 1/0/0（86.0s）和完整 Host script exit 0（402.313s）也仅为历史收据。历史 run `31318855810`（SHA `544c39a`）中 Backend and contracts 成功，iOS 仅 Host smoke 失败：12/1/0，H015 报 `Expected element to exist: body-map.next Button`（exit 65）；CI 无 artifact/accessibility hierarchy，根因未知。当前远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；唯一 H015 未满足旧的 marker-count summary 计数文字断言，不能归因位置丢失。当前工作树使摘要保持可见/可访问，Host 仅检查摘要 identifier 存在，CTA 按 stable-id any-element 验证 exists/hittable/tap；本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 为 2/0/0（136.2s），完整 internal Host script exit 0（421.656s），新修复未推送、远端重试 Pending。不以 `safeAreaInset` 或 `Button` class 作为跨 OS 契约；该工程例外不新增产品样机层，也不替代真机/生产验证，仍须按 [REL-01 当前执行顺序](13_DELIVERY_ROADMAP.md#3-当前执行顺序)关闭生产门禁。
...
- [FEAT-IOS-P0-RUNTIME-01 内部 iOS App Host](28_IOS_P0_RUNTIME_HOST.md)：受版本控制的 Simulator App Host、显式关闭能力集、默认 P0 UI smoke、专用候选 3D probe 与局部 accessibility-size 结构 smoke；基础 smoke、候选 probe 与完整感觉入口均已在本地及 run `31307042209` 的远端 CI 重建。历史 `31314859140`（SHA `98a4200`）的 H015 为 CTA 存在但不可点击；SHA `544c39a` 前的本地 1/0/0（86.0s）和 full Host script exit 0（402.313s）仅历史；历史 `31318855810`（SHA `544c39a`）的 Backend and contracts 成功、iOS 仅 Host smoke 失败（12/1/0，`Expected element to exist: body-map.next Button`，exit 65），CI 无 artifact/hierarchy，根因未知。当前远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；唯一 H015 未满足旧的 marker-count summary 计数文字断言，不能归因位置丢失。当前工作树使摘要保持可见/可访问，Host 对摘要仅检查 identifier 存在，CTA 按 stable-id any-element 验证 exists/hittable/tap；本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 为 2/0/0（136.2s），full internal Host script exit 0（421.656s），新修复未推送、远端重试 Pending。不以 `safeAreaInset`/`Button` class 为契约，internal only。
