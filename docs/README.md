...
- [FEAT-IOS-P0-RUNTIME-01 内部 iOS App Host](28_IOS_P0_RUNTIME_HOST.md)：受版本控制的 Simulator App Host、显式关闭能力集、默认 P0 UI smoke 与专用候选 3D probe；基础 smoke 与候选 probe 均已在本地及 run `31301067572` 的远端 CI 重建，internal only。
- [TEST-IOS-P0-RUNTIME-01 内部 Host 测试计划](29_IOS_P0_RUNTIME_HOST_TEST_PLAN.md)：构建、安装、启动、2D/列表回退、无网络/权限/持久化负向检查和候选 Scene loader-entry/终态；本地和远端 CI 均只形成 Simulator 证据。
