...
- **当前 H015 收据（2026-08-10，以下较早 H015 收据均为历史）。** 远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；唯一 H015 未在 5 秒内满足 `body-map.marker-count-summary` 的旧计数文字断言，不能归因位置丢失。当前工作树使 active/empty count ID 在真实 count pill 上互斥呈现，Host 对摘要只检查 stable identifier 存在；CTA 仍以 any-element `exists`/`isHittable`/`tap` 作为位置已保留的行为证明。本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 2/0/0（138.685s），完整 internal Host script exit 0（441.966s）；本次修复尚未推送、远端重试 Pending。仅内部 Simulator，不构成真机或完整无障碍通过。旧的 136.2s/421.656s 数值均为被本地最新收据取代的历史记录。
- **远端兼容性收据（2026-08-10）。** SHA `9b6e10a` / run `31348638741` 在远端 Xcode 16.4 的 UI test 编译阶段失败：`XCTIssue.isFailure` 在该 XCTest SDK 不可用，0 个 UI test 执行；Backend/contracts、Swift、SDK 与 Host boundary 均已通过。失败诊断 artifact 已上传但没有测试附件；当前兼容性修复改用跨版本 `XCTIssue.type == .assertionFailure`，待提交、推送与重试。该事件不改变 Simulator-only、无障碍、真机或生产门禁边界。
- [FEAT-IOS-P0-RUNTIME-01](28_IOS_P0_RUNTIME_HOST.md) 的仅限内部 Simulator App Host 已有本地与历史远端 CI 收据；`0105cd7` 的 94 项 Core 与 HOST-T-001～014 已由 run `31307042209` 在 iPhone 16 / iOS Simulator 18.5 重建。历史 SHA `98a4200` / run `31314859140` 的 H015 为文字部位 Sheet 关闭后 CTA 存在但不可点击；SHA `544c39a` 前的本地定向 H015 1/0/0（86.0s）和完整 Host script exit 0（402.313s）也仅为历史收据。历史 run `31318855810`（SHA `544c39a`）中 Backend and contracts 成功，iOS 仅 Host smoke 失败：12/1/0，H015 报 `Expected element to exist: body-map.next Button`（exit 65）；CI 无 artifact/accessibility hierarchy，根因未知。当前远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；唯一 H015 未满足旧的 marker-count summary 计数文字断言，不能归因位置丢失。当前工作树使 active/empty count ID 在真实 count pill 上互斥呈现，Host 仅检查摘要 identifier 存在，CTA 按 stable-id any-element 验证 exists/hittable/tap；本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 为 2/0/0（138.685s），完整 internal Host script exit 0（441.966s），本次修复未推送、远端重试 Pending。不以 `safeAreaInset` 或 `Button` class 作为跨 OS 契约；该工程例外不新增产品样机层，也不替代真机/生产验证，仍须按 [REL-01 当前执行顺序](13_DELIVERY_ROADMAP.md#3-当前执行顺序)关闭生产门禁。
...
- [FEAT-IOS-P0-RUNTIME-01 内部 iOS App Host](28_IOS_P0_RUNTIME_HOST.md)：受版本控制的 Simulator App Host、显式关闭能力集、默认 P0 UI smoke、专用候选 3D probe 与局部 accessibility-size 结构 smoke；基础 smoke、候选 probe 与完整感觉入口均已在本地及 run `31307042209` 的远端 CI 重建。历史 `31314859140`（SHA `98a4200`）的 H015 为 CTA 存在但不可点击；SHA `544c39a` 前的本地 1/0/0（86.0s）和 full Host script exit 0（402.313s）仅历史；历史 `31318855810`（SHA `544c39a`）的 Backend and contracts 成功、iOS 仅 Host smoke 失败（12/1/0，`Expected element to exist: body-map.next Button`，exit 65），CI 无 artifact/hierarchy，根因未知。当前远端 `cb5f798` / run `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；唯一 H015 未满足旧的 marker-count summary 计数文字断言，不能归因位置丢失。当前工作树使 active/empty count ID 在真实 count pill 上互斥呈现，Host 对摘要仅检查 stable ID 存在，CTA 按 stable-id any-element 验证 exists/hittable/tap；本地 iPhone 17 Pro / iOS 26.5 targeted H015 + MoreSensations 为 2/0/0（138.685s），full internal Host script exit 0（441.966s），本次修复已在本地完成、待提交推送与远端重试。不以 `safeAreaInset`/`Button` class 为契约，internal only。
...
| TEST-IOS-P0-RUNTIME-01 | [内部 Host 测试计划](29_IOS_P0_RUNTIME_HOST_TEST_PLAN.md) | Host 的 Simulator 构建、启动、P0 路径及负向能力检查 | Historical local HOST-T-001～015 evidence；SHA `544c39a` 前 targeted 1/0/0（86.0s）与 full Host script exit 0（402.313s）仅历史；historical remote `31318855810` (SHA `544c39a`)：Backend and contracts 成功，iOS only Host smoke failed（12/1/0；`Expected element to exist: body-map.next Button`；exit 65），no CI artifact/accessibility hierarchy，根因未知。current remote `cb5f798` / `31321251024` terminal failed：Backend and contracts、Swift、SDK 与 Host boundary 成功，Host smoke 12/1/0；唯一 H015 未满足旧的 marker-count summary count-label assertion，不能归因位置丢失。current worktree keeps mutually exclusive active/empty count identifiers on the real count pill and tests stable identifier existence; stable-id any-element CTA exists/hittable/tap remains the behavior check. iPhone 17 Pro/iOS 26.5 targeted H015 + MoreSensations 2/0/0（138.685s）and full internal Host script exit 0（441.966s）；new fix not pushed/retry Pending; no `safeAreaInset`/`Button` class contract / Simulator-only evidence |
