...
- [FEAT-IOS-P0-RUNTIME-01](28_IOS_P0_RUNTIME_HOST.md) 的仅限内部 Simulator App Host 已在本地与历史远端 CI 重建；`0105cd7` 的 94 项 Core 与 HOST-T-001～014 已由 run `31307042209` 在 iPhone 16 / iOS Simulator 18.5 重建。`7d8bf59` 的首次远端 run `31311301360` 与 `7e64833` 的第二次远端 run `31312416395` 均只失败于 HOST-T-015（均为 exit 65）；第二次运行在 iPhone 16 / iOS 18.5 / Xcode 16.4 中为 12 passed、1 failed、0 skipped，断言未找到 `body-map.next` Button，而后端/契约、Swift 94、iPhoneOS SDK build 和 Host 静态边界均成功。`e0f1089` 仅将该按钮断言移至文字部位 Sheet 关闭后；本地 iPhone 17 Pro / iOS 26.5 的完整 Host suite 与单独 HOST-T-015 均通过，远端复测尚待运行。该工程例外不新增产品样机层，也不替代真机/生产验证；接下来仍须按 [REL-01 当前执行顺序](13_DELIVERY_ROADMAP.md#3-当前执行顺序)关闭生产门禁。
...
| TEST-IOS-P0-RUNTIME-01 | [内部 Host 测试计划](29_IOS_P0_RUNTIME_HOST_TEST_PLAN.md) | Host 的 Simulator 构建、启动、P0 路径及负向能力检查 | Executed locally: HOST-T-001～015；`31311301360` 与 `31312416395` 均仅 HOST-T-015 failed，`e0f1089` local retry passed / remote retry pending / Simulator-only evidence |
