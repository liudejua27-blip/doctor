...
| P0 | 远端版本门禁 | 将已建立的 `codex/initial-git-ci-baseline` 与 CI workflow 配置到远端；首次云端运行绿色，并把检查设为默认分支必需状态 | 提交 `0fa2a17` 已推送且远端默认分支暂为该分支；CI 运行 `31234120644` 两个 job 成功，本地 189 个后端测试、55 个 Swift 测试和静态基线检查绿色；保护分支仍未验证 |
