...
- **当前 H015 收据（2026-08-10）。** 最新远端 `4cc46e2` / run `31350639359` 的 Backend and contracts、Swift tests（94/94）、iPhoneOS SDK build、Host boundary 与 internal Host smoke 全部成功；远端 smoke 运行于 iPhone 16 / iOS Simulator 18.5 / Xcode 16.4。Smoke 脚本在成功时只输出 `Testing started`，不提供精确 XCTest 数。修复后的本地 iPhone 17 Pro / iOS 26.5 targeted H015 1/0/0（77.241s）、多位置 unknown 1/0/0（49.167s），完整 internal Host script exit 0（446.330s；安静输出，不记录精确 XCTest 数）。这些结果仅证明受控 Simulator Host 路径，不构成真机、VoiceOver、完整 Dynamic Type、Reduce Motion、3D 性能/碰撞、候选资产许可、签名、临床或生产通过；旧 `cb5f798` / `31321251024` 等失败保留为历史记录。
- **远端兼容性收据（2026-08-10）。** 历史 SHA `9b6e10a` / run `31348638741` 曾因 `XCTIssue.isFailure` 不兼容而在 UI test 编译阶段失败；当前 `4cc46e2` 已改用跨版本 `XCTIssue.type == .assertionFailure`，并由 run `31350639359` 在 Xcode 16.4 的 iPhone 16 / iOS Simulator 18.5 Host smoke 重建成功。该事件不改变 Simulator-only、无障碍、真机或生产门禁边界。
- [FEAT-IOS-P0-RUNTIME-01](28_IOS_P0_RUNTIME_HOST.md) 的内部 Simulator App Host 已完成当前选择器、bounded scroll 与诊断兼容性修复；`4cc46e2` / run `31350639359` 已在 iPhone 16 / iOS Simulator 18.5 通过完整 CI Host smoke。本地 iPhone 17 Pro / iOS 26.5 也通过 targeted H015、多位置 unknown 和完整 Host script。该结果不以 `safeAreaInset` 或 `Button` class 作为跨 OS 契约，也不替代真机/生产验证；GATE-06/07/08 仍需关闭。
